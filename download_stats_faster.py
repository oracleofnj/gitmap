...
def load_repos(g, data_path):
    try:
        with gzip.open(data_path + "/cached_repos.json.gz", "r") as cached_repos:
            ...
            return repos #OrderedDict(sorted(repos.iteritems(), key=lambda i: -i[1]["stargazers_count"]))
    except IOError:
...
def load_users(g, data_path):
    try:
        with gzip.open(data_path + "/cached_users.json.gz", "r") as cached_users:
            ...
            return users #OrderedDict(sorted(users.iteritems(), key=lambda i: -i[1]["starweight"]))
    except IOError:
...
def get_next(crawlables):
    next_crawlable = next(it.dropwhile(lambda x: x[1]["crawled"], crawlables.iteritems()))
    return next_crawlable[0], next_crawlable[1]

def get_crawl_list(crawlable, sort_by):
    return OrderedDict(sorted([(key, val[sort_by]) for (key, val) in crawlable.iteritems() if not val["crawled"]],
                                key = lambda i:i[1],
                                reverse=True))

def reorder(d):
    return OrderedDict(sorted(d.iteritems(), key = lambda i:i[1], reverse=True))
        ...
        if repo_fullname in to_crawl["repos"]:
            ...
            to_crawl["repos"][repo_fullname] = \
                {"repoObj": star,
                 ...
                 "crawled": False}
            to_crawl["ordered_repos"][repo_fullname] = star.stargazers_count
    to_crawl["ordered_repos"] = reorder(to_crawl["ordered_repos"])

def mark_repo_complete(to_crawl, next_repo_key, success):
    to_crawl["repos"][next_repo_key]["crawled"] = True
    del to_crawl["ordered_repos"][next_repo_key]
    if not success:
...
def mark_user_complete(to_crawl, next_user_key, success):
    ...
    del to_crawl["ordered_users"][next_user_key]
    if not success:
...
def crawl_github(git_uname, git_pw, data_path):
    ...
    while True:
        ...
        to_crawl["ordered_repos"] = get_crawl_list(to_crawl["repos"], "stargazers_count")
        to_crawl["ordered_users"] = get_crawl_list(to_crawl["users"], "starweight")
        i = 0
